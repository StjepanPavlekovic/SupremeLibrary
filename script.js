...
btnSave.addEventListener("click", (e) => {
    ...
    if (validateBookValues()) saveBookData();
});
...
const clearInputs = () => {
    ...
    [errTitle, errAuthor, errPages, errSynopsis].forEach((x) =>
        x.classList.toggle("display-none", true)
    );
};
...
function BookCard(id, element, txtTitle, txtAuthor, txtPages, txtSynopsis) {
    (this.id = id), (this.element = element), (this.txtTitle = txtTitle);
...
let myLibrary = [
    new Book(
        ...
        69,
        true,
        "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quaerat libero facilis architecto consectetur, et maxime fugiat pariatur, quibusdam sit animi."
    ),
    new Book(
        "Book of Water",
        "Some Guy",
...
function beginEditBook(id) {
    ...
    if (currentBook === null || currentBook === undefined)
        window.alert(
            "Suspiciously enough, the book with the given ID was not found..."
        );
